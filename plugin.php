	...
	function search_results()
	{
		...
		if ($paginate == 'yes')
		{
			...
			*/
			...
			// Added by Handri Pangestiaji January 10 2014
			$pagination_config = array();
			...
			// Edit by Handri Pangestiaji January 10 2014
		...
		// -------------------------------------
		// {{ results }} Bypass Added By Handri Pangestiaji January 10 2014
		// -------------------------------------
		// If we don't want to use {{ results }},
		// we don't have to!
		// -------------------------------------

		$loop = false;

		if (preg_match('/\{\{\s?results\s?\}\}/', $this->content()) == 0)
		{
		...
		return $this->streams->parse->parse_tag_content(
								$this->content(), $return, $cache->stream_slug,
								$cache->stream_namespace, $loop, $this->fields);
